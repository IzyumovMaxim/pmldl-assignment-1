...
В модель поступают только признаки, которые можно определить до релиза: описание, жанры, планируемые теги и категории, стартовая цена, платформы, языки и возрастной рейтинг. `Owners`, `CCU`, отзывы и другие пост-релизные показатели не используются как признаки. Результат — статистическая оценка относительно указанного критерия, а не гарантия коммерческого результата.
...
2. **Model engineering:** TF-IDF текста + числовые признаки, логистическая регрессия, метрики и артефакт модели; параметры и результаты логируются в MLflow.
...
curl -X POST http://localhost:8000/predict \
  ...
  -d '{
    "short_description": "A cooperative action adventure in a mysterious world",
...
Результат проверенного запуска на фиксированном test split: ROC-AUC `0.8643`, Average Precision `0.5789`, Brier score `0.0951`. Вероятности калибруются sigmoid-калибровкой на трёх фолдах.
